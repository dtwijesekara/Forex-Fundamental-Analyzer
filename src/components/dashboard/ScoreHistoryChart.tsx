...
export function ScoreHistoryChart() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/history?type=currencies&days=${days}`)
      .then(r => r.json())
      .then(json => {
        if (json.success) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        {loading ? (
          ...
          <div className="h-48 flex items-center justify-center">
            <div className="text-center">
              <p className="text-slate-600 text-sm">Not enough history yet</p>
              <p className="text-slate-700 text-xs mt-1">Run analysis a few times to build up chart data</p>
            </div>
          </div>
        ) : (
        ...
        <div className="flex justify-between text-[9px] text-slate-700 mt-1">
          ...
          <span>+20 / -20 = bias threshold</span>
        </div>
      </CardContent>
    </Card>
  );
...
function CustomTooltip({ active, payload, label }: {
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-2.5 text-[10px] shadow-xl">
...
// ─────────────────────────────────────────────────────────────
// TRANSFORM RAW DB RECORDS → CHART FORMAT
// ─────────────────────────────────────────────────────────────
